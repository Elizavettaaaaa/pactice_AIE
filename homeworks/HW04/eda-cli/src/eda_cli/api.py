...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    # Передаем df, чтобы сработала проверка ID из твоего HW03
    ...
    # Фильтруем только булевы флаги для ответа
    flags_bool = {k: v for k, v in flags_all.items() if isinstance(v, bool)}
...
async def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    Возвращает расширенный набор флагов, включая проверку константных колонок
    и дубликатов ID (логика из HW03).
    """
    try:
        df = pd.read_csv(file.file)
    except Exception as e:
    ...
    # Вызываем твою функцию из HW03
    flags_all = compute_quality_flags(summary, missing_df, df=df)
    
    return {
        "filename": file.filename,
        "quality_score": flags_all.get("quality_score"),
        # Здесь будут видны твои has_constant_columns и has_suspicious_id_duplicates
